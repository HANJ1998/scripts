    ...
    */
    ...
    function openTableViewer(data) {
                ...
                <div class="filter-panel">
                    ...
                    <div class="filter-row preset-row">
                        ...
                        <label class="sort-label" id="sortScoreLabel" style="display:none;">
                            <input type="checkbox" id="sortScoreDesc" onchange="applyScoreSort()"> 降序排列
                        </label>
                    </div>
                    <div class="btn-row">
        ...
        win.generateDropdownOptions = function () {
            ...
            ratingOptions.innerHTML = ratings.map(r =>
                `<label><input type="checkbox" value="${r}" onchange="updateRatingDisplay()"> ${r}</label>`
            ).join('');

            // 生成强赎状态选项（包含"-"代表空值）
            const qiangShuStatuses = win.extractUniqueValues(win.colIndex.qiangShuZhuangTai);
            const qiangShuOptions = win.document.getElementById('qiangShuOptions');
            let optionsHtml = '<label><input type="checkbox" value="-" onchange="updateQiangShuDisplay()"> -（空）</label>';
            optionsHtml += qiangShuStatuses.map(s =>
        ...
        win.renderTable = function (data) {
            ...
            if (data.length === 0) {
                thead.innerHTML = '';
                tbody.innerHTML = '<tr><td style="text-align:center;padding:20px;">没有符合条件的数据</td></tr>';
            ...
            tbody.innerHTML = displayData.slice(1).map(row =>
        ...
        win.calcScoreA = function (row) {
            const xianJia = parseFloat(row[win.colIndex.xianJia]) || 0;
            const yiJiaLv = parseFloat(row[win.colIndex.zhuanGuYiJiaLv]) || 0;
            const guiMo = parseFloat(row[win.colIndex.shengYuGuiMo]) || 0;
        ...
        win.toggleScore = function () {
            ...
            if (score) {
                ...
                win.document.getElementById('sortScoreDesc').checked = false;
            }
        ...
        win.applyScoreSort = function () {
            const desc = win.document.getElementById('sortScoreDesc').checked;
            const data = win.filteredData;
        ...
        win.applyFilter = function () {
            ...
            const premiumMinVal = win.document.getElementById('filterPremiumMin').value;
            const premiumMaxVal = win.document.getElementById('filterPremiumMax').value;
            const premiumMin = premiumMinVal !== '' ? parseFloat(premiumMinVal) : -Infinity;
            const premiumMax = premiumMaxVal !== '' ? parseFloat(premiumMaxVal) : Infinity;
            ...
            const dueDaysMinVal = win.document.getElementById('filterDueDaysMin').value;
            const dueDaysMaxVal = win.document.getElementById('filterDueDaysMax').value;
            const dueDaysMin = dueDaysMinVal !== '' ? parseFloat(dueDaysMinVal) : -Infinity;
            const dueDaysMax = dueDaysMaxVal !== '' ? parseFloat(dueDaysMaxVal) : Infinity;
            const stockName = win.document.getElementById('filterStockName').value.trim();
            const stockNameExclude = win.document.getElementById('filterStockNameExclude').value.trim();
            const stockPriceMinVal = win.document.getElementById('filterStockPriceMin').value;
            const stockPriceMaxVal = win.document.getElementById('filterStockPriceMax').value;
            const stockPriceMin = stockPriceMinVal !== '' ? parseFloat(stockPriceMinVal) : -Infinity;
            const stockPriceMax = stockPriceMaxVal !== '' ? parseFloat(stockPriceMaxVal) : Infinity;

            // 获取选中的评级
            const selectedRatings = [];
            win.document.querySelectorAll('#ratingOptions input:checked').forEach(cb => {
                selectedRatings.push(cb.value);
            });

            // 获取选中的强赎状态
            const selectedQiangShu = [];
            win.document.querySelectorAll('#qiangShuOptions input:checked').forEach(cb => {
            ...
            const filtered = win.tableData.filter((row, index) => {
                ...
                const xianJia = parseFloat(row[win.colIndex.xianJia]) || 0;
                const yiJiaLv = parseFloat(row[win.colIndex.zhuanGuYiJiaLv]) || 0;
                const nianXian = win.parseYears(row[win.colIndex.shengYuNianXian]);
                const guiMo = parseFloat(row[win.colIndex.shengYuGuiMo]) || 0;
                const qiangShuZhuangTai = row[win.colIndex.qiangShuZhuangTai] || '';
                const zuiHouJiaoYiRi = win.parseDaysToDue(row[win.colIndex.zuiHouJiaoYiRi]);
                const pingJia = row[win.colIndex.pingJia] || '';
                const zhengGu = row[win.colIndex.zhengGuMingCheng] || '';
                const zhengGuJia = parseFloat(row[win.colIndex.zhengGuJia]) || 0;
                ...
                if (nianXian >= 0 && (nianXian < yearsMin || nianXian > yearsMax)) return false;
                if (guiMo < scaleMin || guiMo > scaleMax) return false;
                // 强赎状态筛选：支持"-"匹配空值
                if (selectedQiangShu.length > 0) {
                    const actualStatus = qiangShuZhuangTai.trim() === '' ? '-' : qiangShuZhuangTai;
                    if (!selectedQiangShu.includes(actualStatus)) return false;
                }
                if (zuiHouJiaoYiRi < dueDaysMin || zuiHouJiaoYiRi > dueDaysMax) return false;
                if (selectedRatings.length > 0 && !selectedRatings.includes(pingJia)) return false;
                if (stockName && !zhengGu.includes(stockName)) return false;
                if (stockNameExclude && zhengGu.includes(stockNameExclude)) return false;
                if (zhengGuJia < stockPriceMin || zhengGuJia > stockPriceMax) return false;
        ...
        win.updateRatingDisplay = function () {
            ...
            const display = win.document.querySelector('#ratingDropdown .rating-selected');
            display.textContent = selected.length > 0 ? selected.join(', ') : '选择评级';
        };
        ...
        win.updateQiangShuDisplay = function () {
            ...
            const display = win.document.querySelector('#qiangShuDropdown .rating-selected');
            display.textContent = selected.length > 0 ? selected.join(', ') : '选择状态';
        };
        ...
        win.resetFilter = function () {
            ...
            win.document.querySelectorAll('#ratingOptions input').forEach(cb => cb.checked = false);
            win.document.querySelectorAll('#qiangShuOptions input').forEach(cb => cb.checked = false);
            win.document.querySelector('#ratingDropdown .rating-selected').textContent = '选择评级';
            win.document.querySelector('#qiangShuDropdown .rating-selected').textContent = '选择状态';
        ...
        win.getCurrentFilter = function () {
            ...
            return {
                ...
                stockNameExclude: win.document.getElementById('filterStockNameExclude').value,
        ...
        win.applyFilterValues = function (filter) {
            win.document.getElementById('filterPriceMin').value = filter.priceMin || '';
            win.document.getElementById('filterPriceMax').value = filter.priceMax || '';
            win.document.getElementById('filterPremiumMin').value = filter.premiumMin || '';
            win.document.getElementById('filterPremiumMax').value = filter.premiumMax || '';
            win.document.getElementById('filterYearsMin').value = filter.yearsMin || '';
            win.document.getElementById('filterYearsMax').value = filter.yearsMax || '';
            win.document.getElementById('filterScaleMin').value = filter.scaleMin || '';
            win.document.getElementById('filterScaleMax').value = filter.scaleMax || '';
            win.document.getElementById('filterDueDaysMin').value = filter.dueDaysMin || '';
            win.document.getElementById('filterDueDaysMax').value = filter.dueDaysMax || '';
            win.document.getElementById('filterStockName').value = filter.stockName || '';
            win.document.getElementById('filterStockNameExclude').value = filter.stockNameExclude || '';
            win.document.getElementById('filterStockPriceMin').value = filter.stockPriceMin || '';
            win.document.getElementById('filterStockPriceMax').value = filter.stockPriceMax || '';
        ...
        win.deletePreset = function () {
            const name = win.document.getElementById('presetSelect').value || win.document.getElementById('presetName').value.trim();
            if (!name) {
        ...
        win.exportToCSV = function () {
            ...
            const csvContent = data.map((row, rowIndex) =>
            ...
            link.setAttribute('download', '可转债数据_' + new Date().toISOString().slice(0, 10) + '.csv');
        ...
        win.saveScoreBaseline = function () {
            ...
            const rows = data.slice(1);
            const currentScores = rows.map(row => ({
                code: row[win.colIndex.daiMa] || '',
                name: row[win.colIndex.zhengGuMingCheng] || '',
        ...
        win.compareScoreHistory = function () {
            ...
            // 构建对比结果（取前20名）
            const topN = 20;
            const compareResults = currentScores.slice(0, topN).map((item, index) => {
                ...
                <div id="compareModal" style="position:fixed;top:0;left:0;width:100%;height:100%;background:rgba(0,0,0,0.5);z-index:1000;display:flex;align-items:center;justify-content:center;">
                    <div style="background:white;border-radius:8px;max-width:900px;max-height:80vh;overflow:auto;box-shadow:0 4px 20px rgba(0,0,0,0.3);">
                        <div style="padding:15px 20px;background:linear-gradient(135deg, #667eea 0%, #764ba2 100%);color:white;display:flex;justify-content:space-between;align-items:center;">
                            <div>
                                <h3 style="margin:0;">评分A对比结果（升序前${results.length}名）</h3>
                        ...
                        <table style="width:100%;border-collapse:collapse;font-size:13px;">
                            <thead>
                                <tr style="background:#f5f5f5;">
                                    ...
                                    <th style="padding:10px;border-bottom:2px solid #ddd;">名称</th>
                            ...
                            <tbody>
                                ${results.map(r => {
                const scoreChangeColor = r.scoreChange > 0 ? '#f44336' : (r.scoreChange < 0 ? '#4CAF50' : '#666');
                const scoreChangeText = r.scoreChange === null ? '-' : (r.scoreChange > 0 ? '+' + r.scoreChange.toFixed(2) : r.scoreChange.toFixed(2));
                const rankChangeColor = r.rankChange > 0 ? '#4CAF50' : (r.rankChange < 0 ? '#f44336' : '#666');
                const rankChangeText = r.rankChange === null ? '-' : (r.rankChange > 0 ? '↑' + r.rankChange : (r.rankChange < 0 ? '↓' + Math.abs(r.rankChange) : '-'));
                const prevScoreText = r.prevScore === null ? '-' : r.prevScore.toFixed(2);
        ...
        win.copyCompareCodes = function () {
            const codes = win._compareResults.map(r => r.code).join('\n');
            win.navigator.clipboard.writeText(codes).then(() => {
                win.showToast('已复制 ' + win._compareResults.length + ' 个代码', 'success');
            }).catch(() => {
                // 降级方案
        ...
        win.applyCodeFilter = function () {
            ...
            const filtered = win.tableData.filter((row, index) => {
                ...
                const code = row[win.colIndex.daiMa] || '';
