function Workbook_BeforeSave(SaveAsUI, Cancel) {
  ...
  // 【关键】绝对不要加 ThisWorkbook.Saved = true
...
function 名称批量_导出导入() {
  ...
  // 开关1：保留临时表
  let keepSheetIn = InputBox(
    "是否保留【名称导出/名称导入】临时工作表？\n1=保留  2=自动删除",
    "临时表开关",
    "2",
  );
  ...
  let copyRow = 1;
  while (
    tempExportWS.Cells(copyRow, 1).Text !== "" ||
    tempExportWS.Cells(copyRow, 2).Text !== ""
  ) {
